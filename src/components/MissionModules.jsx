...
const modules = [
  {
    ...
    description: 'Responsive products with thoughtful frontends, dependable APIs, and data flows that are easy to evolve.',
  ...
  },
  {
    ...
    description: 'High-impact web experiences that use motion and 3D with purpose, without sacrificing clarity or speed.',
  ...
  },
  {
    ...
    description: 'Repeatable paths from commit to deployment, with practical automation and a focus on resilient releases.',
  ...
  },
  {
    ...
    description: 'Algorithmic thinking applied to real product constraints: performance, maintainability, and useful outcomes.',
    ...
    >
      <div className="mb-8 flex flex-col gap-3 sm:mb-10 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 id="mission-modules-title" className="mt-2 font-heading text-3xl font-bold tracking-tight text-stardust sm:text-4xl">
            What I build
          </h2>
        </div>
        <p className="max-w-sm text-sm leading-relaxed text-cosmos-muted sm:text-right">
          Capabilities mapped to the kind of outcomes a project actually needs—not just a list of tools.
        </p>
      </div>
        ...
        {modules.map((module) => (
          <details key={module.number} className="group p-5 sm:p-6">
            <summary className="flex cursor-pointer list-none items-center gap-4 [&::-webkit-details-marker]:hidden">
              <span className="font-heading text-sm font-bold" style={{ color: module.accent }}>
                {module.number}
              </span>
              <span className="flex-1 font-heading text-base font-semibold text-stardust sm:text-lg">
                {module.title}
              </span>
              <span aria-hidden="true" className="text-xl text-cosmos-muted transition-transform group-open:rotate-45">+</span>
            </summary>
