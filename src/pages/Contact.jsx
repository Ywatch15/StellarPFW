...
export default function Contact() {
  ...
  useSEO({
    title: 'Contact',
    description:
      'Get in touch — send a transmission to collaborate on your next project.',
  });
  ...
  return (
    <>
      ...
      <section
        className="docking-bay mx-auto max-w-2xl px-4 py-12 sm:px-6 sm:py-16"
        aria-label="Contact"
      >
        ...
        <div className="relative mt-6 overflow-hidden rounded-2xl border border-aurora/20 bg-gradient-to-br from-aurora/10 via-nebula/70 to-comet/10 px-4 py-4 shadow-[0_0_30px_rgba(56,189,248,0.06)] sm:px-5">
          <div
            className="pointer-events-none absolute -right-8 -top-10 h-28 w-28 rounded-full border border-aurora/15"
            aria-hidden="true"
          />
          <div
            className="pointer-events-none absolute -right-3 -top-5 h-16 w-16 rounded-full border border-aurora/10"
            aria-hidden="true"
          />
          <div className="relative flex items-start gap-3">
            <span
              role="img"
              className="mt-1 flex h-3 w-3 shrink-0 rounded-full bg-green-400 shadow-[0_0_12px_rgba(74,222,128,0.9)]"
              aria-label="Transmission window open"
            />
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-2 max-w-xl text-sm leading-relaxed text-stardust/75">
                Send a brief about the mission, the problem, and where you want to go
                next. I usually return a signal within{' '}
                <span className="font-semibold text-stardust">1–2 business days</span>.
              </p>
            </div>
          </div>
        </div>
        ...
        >
          <div>
            <label
              htmlFor="name"
              className="mb-1 block text-sm font-medium text-stardust"
            >
              Name
            </label>
            <input
            ...
            />
            {fieldErrors.name && (
              <p id="name-error" className="mt-1 text-xs text-supernova">
                {fieldErrors.name}
              </p>
            )}
          </div>
          ...
          <div>
            ...
            />
            {fieldErrors.email && (
              <p id="email-error" className="mt-1 text-xs text-supernova">
                {fieldErrors.email}
              </p>
            )}
          </div>
          ...
          <div>
            ...
            />
            {fieldErrors.message && (
              <p id="message-error" className="mt-1 text-xs text-supernova">
                {fieldErrors.message}
              </p>
            )}
          </div>
            ...
            >
              ...
              {burst &&
                [1, 2, 3, 4].map((particle) => (
                  <span key={particle} className="docking-particle" aria-hidden="true" />
                ))}
            </button>
          </div>
